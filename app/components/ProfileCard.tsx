...
export default function ProfileCard() {
  const experienceDuration = formatExperienceDuration("2023-08-31");

  const stats = [
    { value: experienceDuration, label: "Experience" },
    ...
    >
      <div className="p-6">
        ...
        <motion.h2
          ...
          className="mt-5 text-center text-3xl font-black tracking-tight text-zinc-900 dark:text-white"
        >
        ...
        >
          <span className="rounded-full bg-zinc-100 px-4 py-1.5 text-sm font-semibold text-zinc-600 ring-1 ring-zinc-200 dark:bg-zinc-800 dark:text-zinc-300 dark:ring-zinc-700">
            Network Engineer
          </span>
        </motion.div>
        ...
        <motion.div
          ...
          className="mt-5 grid grid-cols-3 divide-x divide-zinc-100 rounded-2xl bg-zinc-50 py-4 ring-1 ring-zinc-100 dark:divide-zinc-800 dark:bg-zinc-800/50 dark:ring-zinc-800"
        >
          {stats.map(({ value, label }) => (
            <div key={label} className="flex flex-col items-center gap-0.5 px-2">
              <span className="text-lg font-black" style={{ color: "rgb(var(--accent, 249 115 22))" }}>
                {value}
              </span>
        ...
        {/* Divider */}
        <div className="my-5 h-px w-full bg-gradient-to-r from-transparent via-zinc-200 to-transparent dark:via-zinc-700" />
        ...
        >
          {socials.map(({ label, href, icon: Icon }) => (
            <motion.a
              ...
              className="grid h-11 w-11 place-items-center rounded-full bg-zinc-100 text-zinc-500 transition-colors hover:bg-zinc-200 hover:text-zinc-900 dark:bg-zinc-800 dark:text-zinc-400 dark:hover:bg-zinc-700 dark:hover:text-white"
            >
